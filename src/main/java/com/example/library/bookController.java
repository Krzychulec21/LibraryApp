...
public class bookController {
    ...
    void zatwierdzSzukanie(ActionEvent event) {
        ...
        Wydawnictwo wydawnictwo = wydawnictwoChoiceBox.getSelectionModel().getSelectedItem();
        Gatunek gatunek = gatunekComboBox.getSelectionModel().getSelectedItem();
        tableView.getItems().clear();
        Session session = Connecting.openSession(); // TO DO obsluga szukania gdy nie wybrano autora wydawnictwa agatunku itp - czegos z combo boxa
        String hql = "FROM Ksiazka e where  e.autor = :autor AND e.tytul LIKE :tytul and e.okladka = :okladka and e.rok like :rok and e.wydawnictwo = :wydawnictwo and e.gatunek = :gatunek";
        Query query = session.createQuery(hql);
        query.setParameter("autor", autor);
        query.setParameter("tytul", "%" + tytul + "%");
        query.setParameter("okladka", okladka);
    ...
    private void zaladujTabele() {
        tableView.getItems().clear();
        Session session = Connecting.openSession();
                        ...
                        btnDelete.setOnAction((ActionEvent event) -> {
                            ...
                            if (ksiazka.getStatus().equals("wypozyczona")) {
                                ...
                                if (result.get() == ButtonType.OK) {
                                    // ... user chose OK
                                    Session session2 = Connecting.openSession();
    ...
    void dodajObiekt(ActionEvent event) {
        ...
        if (textField1.getText().isEmpty() || textField1.getText().isBlank() || !textField1.getText().matches("[a-zA-Z]+") || textField2.getText().isEmpty() || textField2.getText().isBlank() || !textField2.getText().matches("[a-zA-Z]+")) {
    ...
    void dodajObiektGatunek(ActionEvent event) {
        ...
        if (textField1.getText().isEmpty() || textField1.getText().isBlank() || !textField1.getText().matches("^[a-zA-Z\\s]*$")) {
            errorTextAddObjects.setText("Pole nie może byc puste i musi zawierać same litery!");
            errorTextAddObjects.setVisible(true);
        } else {
